...
use ::rr::{DNSClass, Name, Record, RData, RecordType};
...
impl Signer {
    ...
    if let &RData::SIG { num_labels, .. } = rrsig_rdata {
...

      {
        ...
        for record in rrset {
          ...
          //
          //                type is the RRset type and all RRs in the class
          assert!(rr_type.emit(&mut encoder).is_ok());
          //
          //                OrigTTL is the value from the RRSIG Original TTL field
          assert!(encoder.emit_u32(record.get_ttl()).is_ok());
          //
          //                All names in the RDATA field are in canonical form (set above)
          assert!(record.get_rdata().emit(&mut encoder).is_ok());
        }
      }
      ...
      let mut hasher = Hasher::new(self.algorithm.get_hash_type());
      assert!(hasher.write_all(&buf).is_ok());
      self.pkey.sign_with_hash(&hasher.finish() , self.algorithm.get_hash_type())
    } else {
      // TODO: this should be an error
      vec![]
    }
  ...
  fn determine_name(name: &Name, num_labels: u8) -> Option<Name> {
    ...
    if num_labels < fqdn_labels {
      ...
      if let Some(rightmost) = name.base_name() {
...
fn test_sign_rrset() {
  ...
  let sig = signer.sign_rrset(&rrsig, &rrset);
  assert!(!sig.is_empty());
  ...
  let filtered_sig = signer.sign_rrset(&rrsig, &rrset);
  assert!(!sig.is_empty());
  assert_eq!(sig, filtered_sig);
}
