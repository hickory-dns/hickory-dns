...
pub struct SecureDnsHandle<H: ClientHandle + 'static> {
    client: H,
    ...
    H: ClientHandle + 'static,
{
    ...
    pub fn with_trust_anchor(client: H, trust_anchor: TrustAnchor) -> SecureDnsHandle<H> {
        SecureDnsHandle {
            client: client,
...
impl<H> DnsHandle for SecureDnsHandle<H>
where
    H: ClientHandle,
{
    ...
    fn send(&mut self, mut message: Message) -> Box<Future<Item = Message, Error = Self::Error>> {
        ...
        if let OpCode::Query = message.op_code() {
            ...
            let client: SecureDnsHandle<H> = self.clone_with_context();
...
impl<H> ClientHandle for SecureDnsHandle<H>
where
    H: ClientHandle + 'static,
{
