...
impl SocketPorts {
    ...
    pub fn put<T: Into<ServerProtocol>>(&mut self, protocol: T, addr: SocketAddr) {
    ...
    pub fn get_v4<T: Into<ServerProtocol>>(&self, protocol: T) -> Option<u16> {
        self.0
            .get(&protocol.into())
            .iter()
    ...
    /// Assumes there is only one V4 addr for the IP based on the usage in the Server
    #[allow(unused)]
    pub fn get_v6<T: Into<ServerProtocol>>(&self, protocol: T) -> Option<u16> {
        self.0
            .get(&protocol.into())
            .iter()
