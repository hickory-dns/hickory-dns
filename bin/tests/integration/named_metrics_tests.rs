...
fn test_metrics_description_present(scrape: Scrape) {
    let missing = scrape
        ...
        .into_iter()
        .filter_map(|s| {
            // Allow "undocumented" metrics ending with _sum or _count - these are generated by
            // the describe_histogram! macro, and despite providing a description, are not described
            // in the scraped docs.
            let histogram_metric = s.metric.ends_with("_sum") || s.metric.ends_with("_count");
            if !scrape.docs.contains_key(&s.metric) && !histogram_metric {
                Some(s.metric)
            } else {
                None
            }
