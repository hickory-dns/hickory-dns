...
pub(crate) fn packet_loss_handler(
    bytes: &[u8],
    _transport: Transport,
) -> Result<Option<Vec<u8>>, ProtoError> {
    let mut msg = Message::from_vec(bytes)?.to_response();
...
pub(crate) fn cname_loop_handler(
    bytes: &[u8],
    _transport: Transport,
) -> Result<Option<Vec<u8>>, ProtoError> {
    let mut msg = Message::from_vec(bytes)?.to_response();
    ...
    let tokens = host_str.split("-").collect::<Vec<&str>>();
    let round = if tokens.len() != 3 {
        0
    } else {
        tokens[1].parse::<usize>().unwrap() + 1
    };
