...
impl UdpStream {
  ...
  pub fn new(name_server: SocketAddr, loop_handle: Handle) -> (Box<Future<Item=UdpStream, Error=io::Error>>, BufStreamHandle) {
    ...
    let stream: Box<Future<Item=UdpStream, Error=io::Error>> = Box::new(next_socket
...
fn udp_stream_test(server_addr: std::net::IpAddr) {
  ...
  let server_handle = std::thread::Builder::new().name("test_udp_stream_ipv4:server".to_string()).spawn(move || {
