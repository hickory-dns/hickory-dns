...
impl TlsStream {
  ...
  fn new(certs: Vec<OpensslX509>, pkcs12: Option<Pkcs12>) -> io::Result<TlsConnector> {
    ...
    { // mutable reference block
      ...
      for cert in certs {
        try!(store.add_cert(cert).map_err(|e| io::Error::new(io::ErrorKind::ConnectionRefused, format!("tls error: {}", e))));
      }

      try!(openssl_ctx_builder.set_verify_cert_store(store.build()).map_err(|e| io::Error::new(io::ErrorKind::ConnectionRefused, format!("tls error: {}", e))));
    }
