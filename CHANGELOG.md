...
but three new types were introduced: `Client`, `SecureClient`, and the
`SyncClient` trait. Both types are synchronous implementations of the old `Client`
 function interfaces. Please read those docs on those new types and the
`SyncClient` trait.
