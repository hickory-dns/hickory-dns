...
pub(crate) struct RecursorPool<P: ConnectionProvider> {
    ...
    #[cfg(feature = "metrics")]
    iterative_query_counter: Counter,
}

impl<P: ConnectionProvider> RecursorPool<P> {
    pub(crate) fn from(zone: Name, ns: NameServerPool<P>) -> Self {
        #[cfg(feature = "metrics")]
        let iterative_query_counter = counter!("hickory_iterative_queries_total");
        #[cfg(feature = "metrics")]
        describe_counter!(
            "hickory_iterative_queries_total",
            Unit::Count,
            "Number of iterative queries made during resolution."
        );
    ...
    pub(crate) async fn lookup(
        ...
        let lookup = self
            ...
            .or_insert_with(move || {
                ...
                #[cfg(feature = "metrics")]
                self.iterative_query_counter.increment(1);
