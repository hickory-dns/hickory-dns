...
/*#[cfg(feature = "tokio-runtime")]
/// A new type of `tokio::net::TcpStream` that implements `std::io::AsyncRead` and `std::io::AsyncWrite`
pub struct DnsTokioTcpStream(TokioTcpStream);

#[cfg(feature = "tokio-runtime")]
impl AsyncRead for DnsTokioTcpStream {
    fn poll_read(
...
#[cfg(feature = "tokio-runtime")]
impl AsyncWrite for DnsTokioTcpStream {
    fn poll_write(
        mut self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        buf: &[u8],
...
mod tests {
    ...
    fn test_tcp_stream_ipv6() {
        let io_loop = Runtime::new().expect("failed to create tokio runtime");
        tcp_client_stream_test::<DnsTokioTcpStream, Runtime, TokioTime>(
