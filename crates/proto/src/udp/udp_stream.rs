...
pub(crate) struct NextRandomUdpSocket<S> {
    ...
    state: Option<(UdpSocketCreateAttempt, UdpSocketCreateFuture<S>)>,
}

type UdpSocketCreateAttempt = usize;
                    ...
                    match fut.as_mut().poll(cx) {
                        ...
                        Poll::Ready(Err(err)) => match err.kind() {
                            io::ErrorKind::AddrInUse
                                if attempt > 0 && attempt != UdpSocketCreateAttempt::MAX =>
                            {
