...
impl<P: ConnectionProvider> NameServerState<P> {
    ...
    async fn send(self: Arc<Self>, request: DnsRequest) -> Result<DnsResponse, ProtoError> {
        let mut prev = self.client.lock().await;

        // if this is in a failure state
        let client = match (&*prev, self.status()) {
            // if the client is already connected, then use it
            (Some(client), Status::Init | Status::Established) => {
            ...
            _ => {
                ...
                *prev = Some(new_client.clone());
                new_client
            }
        };
        drop(prev); // drop the lock before we start the request
...
mod tests {
    ...
    use crate::proto::xfer::{DnsRequestOptions};
