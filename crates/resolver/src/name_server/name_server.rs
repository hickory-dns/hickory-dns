...
impl<P: ConnectionProvider> NameServer<P> {
    ...
    async fn connected_mut_client(
        ...
        if let Some(conn) = policy.select_connection(
            self.config.ip,
            &*cx.encrypted_transport_state.0.lock().await,
    ...
    async fn consider_probe_encrypted_transport(
        ...
        let should_probe = {
            let state = cx.encrypted_transport_state.0.lock().await;
    ...
    fn probe_encrypted_transport(
        ...
        let encrypted_transport_state = cx.encrypted_transport_state.clone();
        let budget = cx.opportunistic_probe_budget.clone();
...
mod tests {
    ...
    async fn test_opportunistic_probe() {
        ...
        let name_server = NameServer::new([].into_iter(), config, &opts, mock_provider.clone());
