...
pub(crate) fn client_config() -> Result<Arc<ClientConfig>, ProtoError> {
    #[cfg(not(all(feature = "native-certs", not(feature = "webpki-roots"))))]
    {
        ...
        static CONFIG: Lazy<Result<Arc<ClientConfig>, ProtoError>> =
            Lazy::new(internal_client_config);
        CONFIG.clone()
    }
...
fn internal_client_config() -> Result<Arc<ClientConfig>, ProtoError> {
...
mod tests {
    ...
    fn test_adguard_quic() {
        ...
        let mut config = super::internal_client_config().unwrap();
        Arc::get_mut(&mut config).unwrap().enable_sni = true;
