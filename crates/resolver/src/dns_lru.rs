...
pub struct TtlBounds {
    ...
    #[cfg_attr(feature = "serde", serde(default, with = "duration_deserialize"))]
    positive_min_ttl: Option<Duration>,
...
/// This module provides an alternate deserialization method for a [`Duration`] that expects a
/// single number, representing the number of seconds, instead of a struct with `secs` and `nanos`
/// fields.
#[cfg(feature = "serde")]
mod duration_deserialize {
    use std::{
        fmt::{self, Formatter},
        time::Duration,
    };
    ...
    where
        D: Deserializer<'de>,
    {
        deserializer.deserialize_option(OptionDurationSecondsVisitor)
    }

    struct OptionDurationSecondsVisitor;

    impl<'de> Visitor<'de> for OptionDurationSecondsVisitor {
        ...
        where
            D: Deserializer<'de>,
        {
            Ok(Some(deserializer.deserialize_u32(DurationSecondsVisitor)?))
        }
    }
