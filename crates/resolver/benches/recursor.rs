...
/// A two-level mock DNS hierarchy:
///
///   . (ROOT_IP)
///   └── testing.       (TLD_IP)
...
fn make_provider() -> MockProvider {
    ...
    MockProvider::new(MockNetworkHandler::new(vec![
        // Root → TLD delegation
...
fn make_recursor(provider: MockProvider) -> Recursor<MockProvider> {
...
fn bench_warm_resolve(c: &mut Criterion) {
    ...
    // Arc lets the Recursor be shared across the 'static async closures that
    // criterion's iter() requires.
    let recursor = Arc::new(make_recursor(make_provider()));
