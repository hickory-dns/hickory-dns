...
impl<P: RuntimeProvider + Send + Sync> ZoneHandler for InMemoryZoneHandler<P> {
    ...
    async fn lookup(
        ...
        let is_cname_answer = answer.as_ref().is_some_and(|a| {
            a.record_type() == RecordType::CNAME && query_type != RecordType::CNAME
        });

        let (answer, cname_chain) = if is_cname_answer {
            let chain = inner.chase_cnames(name, answer.unwrap(), query_type, lookup_options);
