...
trait StoreBackendExt: StoreBackend {
    ...
    fn soa(&self, origin: &LowerName) -> Option<&SOA> {
        let rrset = self.get_rrset(origin, RecordType::SOA)?;
        let record = rrset.records_without_rrsigs().next()?;
        record.data().as_soa()
    }

    /// Returns the minimum TTL from the SOA record.
    #[cfg(feature = "__dnssec")]
    fn minimum_ttl(&self, origin: &LowerName) -> u32 {
        if let Some(soa) = self.soa(origin) {
            soa.minimum()
        } else {
            error!(%origin, "could not look up SOA for authority");
            0
        }
    }
    ...
    fn upsert(&mut self, record: Record, serial: u32, dns_class: DNSClass) -> bool {
        ...
        let records: &mut Arc<RecordSet> = self.records_mut().entry(rr_key).or_insert_with(|| {
    ...
    fn sign_zone(
        ...
        let (records, secure_keys) = self.as_mut_tuple();
    ...
    #[cfg(feature = "__dnssec")]
    fn closest_nsec(&self, name: &LowerName) -> Option<Arc<RecordSet>> {
    ...
    fn replace_any(&self, name: &LowerName) -> RecordType {
