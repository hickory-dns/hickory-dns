...
#[cfg(feature = "metrics")]
use metrics::ReportingResponseHandlerMetrics;
...
struct ReportingResponseHandler<R: ResponseHandler> {
    ...
    handler: R,
    #[cfg(feature = "metrics")]
    metrics: ReportingResponseHandlerMetrics,
}
...
impl<R: ResponseHandler> ResponseHandler for ReportingResponseHandler<R> {
    async fn send_response<'a>(
        ...
        #[cfg(feature = "metrics")]
        {
            self.metrics.request_proto.increment(&self.protocol);
            self.metrics
                .request_operation
                .increment(&self.request_header.op_code());
            self.metrics.request_flags.increment(&self.request_header);

            self.metrics.response_code.increment(&response_code);
            self.metrics.response_flags.increment(&response_info)
        }
...
pub(crate) async fn handle_request<R: ResponseHandler, T: RequestHandler>(
    ...
    let inner_handle_request = |message: MessageRequest, response_handler: R| async move {
        ...
        let reporter = ReportingResponseHandler {
            ...
            #[cfg(feature = "metrics")]
            metrics: ReportingResponseHandlerMetrics::default(),
        };
