...
pub(super) struct CatalogMetrics {
    zone_store_metrics: HashMap<&'static str, ZoneLookupMetrics>,
...
impl CatalogMetrics {
    pub(super) fn add_authority(&mut self, authority: &dyn Authority) {
        self.zone_store_metrics.insert(
            authority.metrics_label(),
            ZoneLookupMetrics::new(authority.metrics_label(), authority.zone_type()),
        );
    }

    pub(super) fn remove_authority(&mut self, authority: &dyn Authority) {
        self.zone_store_metrics.remove(authority.metrics_label());
    }
    ...
    pub(super) fn update_zone_lookup(
        ...
        let Some(zone_store_metrics) = self.zone_store_metrics.get(authority.metrics_label())
        else {
            return;
        };
