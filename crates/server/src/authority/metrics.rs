...
use crate::authority::{AuthorityObject, ZoneType};

pub(super) struct CatalogMetrics {
    zone_metrics: ZoneTypeMetrics,
...
impl CatalogMetrics {
    pub(super) fn update<'a>(
        &self,
        authority: Arc<dyn AuthorityObject>,
        query: &LowerQuery,
        answers: impl Iterator<Item = &'a Record> + Send + 'a,
    ) {
        self.zone_metrics.increment(authority.zone_type());
...
impl Default for CatalogMetrics {
    fn default() -> Self {
        Self {
            zone_metrics: ZoneTypeMetrics::default(),
...
struct ZoneTypeMetrics {
    authoritative_primary: Counter,
    authoritative_secondary: Counter,
    external_forwarded: Counter,
}

impl ZoneTypeMetrics {
    fn increment(&self, zone_type: ZoneType) {
        #[allow(deprecated)]
        match zone_type {
            ZoneType::Primary | ZoneType::Master => self.authoritative_primary.increment(1),
            ZoneType::Secondary | ZoneType::Slave => self.authoritative_secondary.increment(1),
            ZoneType::External => self.external_forwarded.increment(1),
        }
    }
}

impl Default for ZoneTypeMetrics {
    fn default() -> Self {
        let zone_types_name = "hickory_zone_lookups_total";
        let key = "type";
        let role = "role";

        Self {
            authoritative_primary: {
                let new = counter!(zone_types_name, key => "authoritative", role =>  "primary");
                describe_counter!(zone_types_name, Unit::Count, "number of zone lookups");
                new
            },
            authoritative_secondary: counter!(zone_types_name, key => "authoritative", role => "secondary"),
            external_forwarded: counter!(zone_types_name, key => "external", role => "forwarded"),
        }
    }
}
...
impl DNSClassMetrics {
    fn new(direction: &'static str) -> Self {
        let dns_class_name = format!("hickory_{}_dns_classes_total", direction);
        let key = "class";
        Self {
            r#in: {
            ...
            opt: counter!(dns_class_name.clone(), key => "opt"),
    ...
    fn increment(&self, dns_class: DNSClass) {
        match dns_class {
            ...
            DNSClass::OPT(_) => self.opt.increment(1),
...
impl RecordTypeMetrics {
    fn new(direction: &'static str) -> Self {
        let record_type_name = format!("hickory_{}_record_types_total", direction);
        let key = "type";
        ...
        Self {
            a: {
                ...
                describe_counter!(
                    ...
                    Unit::Count,
                    format!("number of {} record types", direction)
                );
                new
            },
    ...
    fn increment(&self, record_type: RecordType) {
        match record_type {
            ...
            RecordType::Unknown(_) => self.unknown.increment(1),
