...
use crate::proto::{
    ...
    rr::Record,
    serialize::binary::{BinDecodable, BinDecoder, BinEncodable, BinEncoder, NameEncodingMode},
};
...
impl EmitAndCount for QueriesEmitAndCount<'_> {
    fn emit(&mut self, encoder: &mut BinEncoder<'_>) -> Result<usize, ProtoError> {
        ...
        if matches!(encoder.name_mode(), NameEncodingMode::Compressed) && self.first_query.is_some()
        {
