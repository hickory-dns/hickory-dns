...
impl Decompressible for Minfo {
    ...
    fn decompress(compressed_rdata: &[u8], message: &[u8]) -> Self {
        ...
        let rmailbx_decompressed = Name::decompress(compressed_rdata, message);
        let emailbx_decompressed = Name::decompress(&compressed_rdata[emailbx_offset..], message);

        Minfo {
            rmailbx: rmailbx_decompressed,
            emailbx: emailbx_decompressed,
        }
    }
}
...
impl Decompressible for Mx {
    fn decompress(input: &[u8], message: &[u8]) -> Self {
        ...
        let exchange_decompressed = Name::decompress(&input[2..], message);
        Self {
            preference,
            exchange: exchange_decompressed,
        }
    }
}
...
impl Decompressible for Soa {
    ...
    fn decompress(compressed_rdata: &[u8], message: &[u8]) -> Self {
        ...
        let mname_decompressed = Name::decompress(compressed_rdata, message);
        let rname_decompressed = Name::decompress(&compressed_rdata[rname_offset..], message);

        let rest = compressed_rdata[serial_offset..].to_vec();

        Soa {
            mname: mname_decompressed,
            rname: rname_decompressed,
            rest,
        }
    }
}
