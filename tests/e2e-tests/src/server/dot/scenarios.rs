...
/// with DoT probing. After the recursor successfully discovers the HickoryDNS auth servers support
...
fn pdns_opportunistic_dot_success() -> Result<()> {
    ...
    // Make a recursive query for the root FQDN using the resolver instance.
    ...
    // Then, make a second unique query for a different name to provoke DoT probing.
    // The pdns-recursor implementation won't do this based on the first query alone.
    let resp = client.dig(query_settings, resolver_ip, RecordType::A, &FQDN::TEST_TLD)?;
    assert!(resp.status.is_noerror());
    ...
    // Now, make a final unique query for yet another name. The recursor should use DoT to satisfy
    // the request based on its probing success.
    let resp = client.dig(query_settings, resolver_ip, RecordType::MX, &FQDN::TEST_TLD)?;
    assert!(resp.status.is_noerror());

    // Wait until we've seen an incoming DoT query for the correct record name.
    // This is the query from the resolver that was upgraded to DoT opportunistically.
    let test_tld_name = FQDN::TEST_TLD.as_str().trim_end_matches('.');
    tshark.wait_until(
        |captures| {
            captures.iter().any(|c| {
                matches!(c.direction, Direction::Incoming { .. })
                    && c.dst_port == DOT_PORT
                    && query_name(c) == test_tld_name
            })
        },
        Duration::from_secs(10),
    )?;
    ...
    //  * Our second query, a lookup for the testing TLD A record.
    assert_eq!(udp_queries.len(), 2);
    assert_eq!(query_name(&udp_queries[0]), "<Root>");
    assert_eq!(query_name(&udp_queries[1]), test_tld_name);
    ...
    //    second query.
    //  * Our third and final query, a lookup for the testing domain's A record. We verified
...
// Perform serde_json::Value acrobatics to extract `dns.qry.name` from the first query.
fn query_name(c: &Capture) -> &str {
    ...
    first_query.get("dns.qry.name").unwrap().as_str().unwrap()
}
