...
pub fn insecure_record_fixture(
    ...
    let mut insecure_ns = NameServer::new(
        &Implementation::test_peer(),
        insecure_zone.clone(),
        &network,
    )?;
    insecure_ns.add(Record::a(needle_fqdn.clone(), needle_ipv4_addr));
    ...
    let _insecure_ns = insecure_ns.start()?;
