...
async fn test_tsig_zone_transfer() {
    ...
    let mut zone_file = ZoneFile::new(SOA {
        ...
        ttl: 86400,
        nameserver: FQDN::from_str("root.example.net.").unwrap(),
    ...
    zone_file.add(DnsTestRecord::ns(
        FQDN::from_str("example.net.").unwrap(),
        FQDN::from_str("root.example.net.").unwrap(),
    ));

    // Add A record for the nameserver itself
    zone_file.add(DnsTestRecord::a(
        FQDN::from_str("root.example.net.").unwrap(),
        ns.ipv4_addr(),
    ));
    ...
    ns.add_zone(FQDN::from_str("example.net.").unwrap(), zone_file);
    ns.tsig_key(&tsig_key);
    let ns = ns.start().unwrap();
...
fn tsig_key_and_signer() -> (TsigKey, Arc<TSigner>) {
    let tsig_key = TsigKey {
        ...
        secret_key: "!! t0p $3cr3t !!".as_bytes().to_vec(),
    };
    let signer = Arc::new(
        TSigner::new(
            tsig_key.secret_key.clone(),
